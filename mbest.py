...
class gim(RandomForestRegressor):
    '''
        This class completely inherit scikit-learn's RandomForestRegressor.
        I provide two additional function to compute feature importance. One
        is compute_feature_importance and another is traverse which helps
        compute_feature_importance to traverse every tree in the forest.
    '''
    ...
    def __init__(self,
                 ...
                 warm_start=False):
        '''
        These parameters are completely same with the function 'RandomForestRegressor', you can
        refer to help document of sklearn for more detailed information.
        '''
    ...
    def compute_feature_importance(self, x, y, group_by = None, std = 'Y', n_jobs = None):
        ...
        :param group_by: used for group the data and a column of data that can be hashed, but is not
        necessary. You can group the data in advance instead.
    ...
    def traverse(self,tree,x,y,y_squared,data_choose_bool):
        '''
        This function is to compute feature importance for each group in one tree.
        ...
        for inde in range(self.FL):
            ...
            # to select node which containing more one record. Because there is no squared error if the node has less than tow records.
            data_num_lar = (data_num > 1)
            # to obtain to index of node which containing more one record.
...
gvi = gim(500,max_features=5,n_jobs=-30)# Generate random forests model
